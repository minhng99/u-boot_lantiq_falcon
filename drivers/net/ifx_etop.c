...
static int ltq_eth_send(struct eth_device *dev, volatile void *packet,int length);
static int ltq_eth_recv(struct eth_device *dev);
static void ltq_eth_halt(struct eth_device *dev);
static void ltq_eth_init_chip(void);
static void ltq_eth_init_dma(void);

static int ltq_eth_miiphy_read(const char *devname, u8 phyAddr, u8 regAddr, u16 * retVal)
{
	u32 timeout = 50000;
	u32 phy, reg;
...
static int ltq_eth_miiphy_write(const char *devname, u8 phyAddr, u8 regAddr, u16 data)
{
	u32 timeout = 50000;
	u32 phy, reg;
...
int ltq_eth_initialize(bd_t * bis)
{
	struct eth_device *dev;

	debug("Entered ltq_eth_initialize()\n");
	...
	miiphy_register(dev->name,
			ltq_eth_miiphy_read, ltq_eth_miiphy_write);
#endif
...
static int ltq_eth_init(struct eth_device *dev, bd_t * bis)
{
	...
	debug("ltq_eth_init %x:%x:%x:%x:%x:%x\n",
...
#ifdef DEBUG
static void ltq_dump(const u8 *data, const u32 length)
{
	u32 i;
	debug("\n");
	for(i=0;i<length;i++) {
...
#ifdef DEBUG
	ltq_dump(tx_desc->DataPtr, tx_desc->status.field.DataLen);
#endif
...
static int ltq_eth_recv(struct eth_device *dev)
{
	...
	debug("rx");
#ifdef DEBUG
	ltq_dump(rx_desc->DataPtr, rx_desc->status.field.DataLen);
#endif
